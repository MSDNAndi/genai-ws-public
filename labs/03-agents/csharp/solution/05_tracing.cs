...
agent = agent.AsBuilder().UseOpenTelemetry(Source, configure: o => o.EnableSensitiveData = true).Build();
tracing = Sdk.CreateTracerProviderBuilder()
    .ConfigureResource(resource => resource.AddService("bsgai-lab3"))
    .AddSource(Source)
    .AddProcessor(new BatchActivityExportProcessor(new OneLinePerSpan()))
    .Build();
// <<< TODO
